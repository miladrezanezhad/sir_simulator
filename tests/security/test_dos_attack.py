...
class TestDoSAttack(unittest.TestCase):
    ...
    def test_extreme_time_steps(self):
        ...
        try:
            ...
            df = run_sir_simulation(
                beta=0.5, gamma=0.2,
                S0=990, I0=10, R0=0,
                t_max=10000, steps=50000  # 50,000 steps
            )
            elapsed = time.time() - start_time
            self.assertLess(elapsed, 60)  # Should complete within 60 seconds
    ...
    def test_extreme_population_size(self):
        ...
        try:
            df = run_sir_simulation(
                beta=0.5, gamma=0.2,
                S0=10_000_000, I0=1000, R0=0,  # 10 million population
                t_max=100, steps=500
            )
            self.assertGreater(len(df), 0)
        except MemoryError:
            ...
            # Acceptable if simulation takes too long, but not crash
            self.assertIn(str(e), ["", "overflow"])
    ...
    def test_network_with_too_many_nodes(self):
        ...
        try:
            ...
            df = sim.simulate_spread(transmission_prob=0.3, recovery_prob=0.1, max_steps=20)
            self.assertGreater(len(df), 0)
        except MemoryError:
    ...
    def test_extreme_parameter_values(self):
        ...
        extreme_params = [
            (0, 0),      # Zero transmission and recovery
            (2, 2),      # Very high values
            (-0.5, 0.1), # Negative beta (invalid but should not crash)
            (0.5, -0.1), # Negative gamma
            (None, 0.1), # Invalid type
        ]
        
        for beta, gamma in extreme_params:
            try:
                if beta is None:
                    beta = "invalid"  # Test string input
                df = run_sir_simulation(
                    beta=beta, gamma=gamma,
                    S0=990, I0=10, R0=0,
                    t_max=10, steps=50
                )
                # If it runs without crash, that's fine
                self.assertTrue(True)
            except (TypeError, ValueError):
                # Expected exception for invalid types/values
                pass
            except Exception as e:
