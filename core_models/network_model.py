...
class SocialNetworkSimulator:
    ...
    def simulate_spread(self, transmission_prob=0.3, recovery_prob=0.1, 
                        initial_infected=5, max_steps=50):
        """
        ...
        initial_infected_nodes = np.random.choice(list(all_nodes), 
                                                   size=min(initial_infected, self.num_nodes), 
                                                   replace=False)
    ...
    def visualize_network(self, show_labels=False):
        ...
        nx.draw(self.graph, pos, node_color=node_colors, 
                node_size=50, with_labels=show_labels, 
