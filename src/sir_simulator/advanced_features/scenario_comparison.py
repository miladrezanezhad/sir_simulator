...
class ScenarioComparator:
    ...
    def baseline_scenario(self, days=150):
        ...
        result = odeint(
            self.sir_model, self.initial_conditions, t, args=(self.beta, self.gamma)
        )
        return pd.DataFrame(
            {
                "day": t,
    ...
    def quarantine_scenario(
        self, days=150, start_day=10, end_day=80, reduction_factor=0.2
    ):
        def sir_quarantine(y, t, beta, gamma):
            S, I, R = y
            effective_beta = (
                beta * reduction_factor if start_day <= t <= end_day else beta
            )
        ...
        t = np.linspace(0, days, days)
        result = odeint(
            sir_quarantine, self.initial_conditions, t, args=(self.beta, self.gamma)
        )
        return pd.DataFrame(
            {
                "day": t,
    ...
    def vaccination_scenario(
        self, days=150, start_day=5, daily_vaccinations=15, vaccine_efficacy=0.9
    ):
        ...
        t = np.linspace(0, days, days)
        result = odeint(
            sir_vaccination, self.initial_conditions, t, args=(self.beta, self.gamma)
        )
        return pd.DataFrame(
            {
                "day": t,
    ...
    def combined_scenario(
        ...
        t = np.linspace(0, days, days)
        result = odeint(
            sir_combined, self.initial_conditions, t, args=(self.beta, self.gamma)
        )
        return pd.DataFrame(
            {
                "day": t,
    ...
    def compare_all_scenarios(self, days=150):
        ...
        for name, df in scenarios.items():
            ...
            reduction = (
                0
                if name == "Baseline"
                else (baseline_peak - peak) / baseline_peak * 100
            )
            metrics.append(
                {
                    "scenario": name,
